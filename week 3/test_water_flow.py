'''
To exceed the assignment requirements, I wrote a new test function to verify the 
correctness of the convert_kpa_to_psi function I included in the waterflow.py file.
'''

from pytest import approx
import pytest
from waterflow import water_column_height, pressure_loss_from_pipe, \
    pressure_gain_from_water_height, pressure_loss_from_fittings, \
        reynolds_number, pressure_loss_from_pipe_reduction, convert_kpa_to_psi
...
def test_reynolds_number():
    assert reynolds_number(0.048682, 0.0) == approx(0.0, abs=1)
...
def test_convert_kpa_to_psi():
    ...
    assert convert_kpa_to_psi(6.895) == approx(1.0, abs=0.1)
    assert convert_kpa_to_psi(34.475) == approx(5.0, abs=0.1)
    assert convert_kpa_to_psi(68.95) == approx(10.0, abs=0.1)
    assert convert_kpa_to_psi(137.9) == approx(20.0, abs=0.1)
    assert convert_kpa_to_psi(206.85) == approx(30.0, abs=0.1)
